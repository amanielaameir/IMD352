...
export default class CourseSearch extends React.Component {
    ...
    componentDidMount() {
        ...
        let url = settings.sws.baseURL + "/student/v5/course.json?year=2010&quarter=autumn&future_terms=0&curriculum_abbreviation=D+HYG&course_number=&course_title_starts=&course_title_contains=&page_size=10";
    ...
    render() {
        ...
        return (
            <Container>
                ...
                <div>
                    <ul>
                        {(this.state.results != null) ? this.state.results.Courses.map((item, key) =>
                            (<li key={key}>{item.CourseTitle} : {item.CourseTitleLong}</li>)
                        ) : "Waiting..."}
                    </ul>
                </div>
            </Container>
